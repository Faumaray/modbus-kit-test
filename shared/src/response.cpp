...
Response::Response(uint8_t slaveId, FunctionCode functionCode,
  ...
  m_body.reserve(values.size() * 2);
  for (auto &value : values) {
    auto split = split_16_to_Hi_and_Lo(value);
    m_body.emplace_back(split[0]);
    m_body.emplace_back(split[1]);
  }
}
...
Response::Response(std::vector<uint8_t> inputData) {
  ...
  if (inputData[0] >= 0x80) {
    ...
    for (auto i = 3; i < inputData.size(); i++) {
...
std::ostream &operator<<(std::ostream &os, const Response &response) {
  if (std::holds_alternative<FunctionCode>(response.m_header.FunctionCode)) {
        os << std::get<FunctionCode>(response.m_header.FunctionCode) << ", from slave " << response.m_header.slaveID;
        os << ", returned next " << std::get<std::uint8_t>(response.m_header.info) << " values";
        for(const auto &value : response.m_body){
            os << "\n value = " << value;
        }
  }else {
        std::string res;
        os << "Error on slave " << response.m_header.slaveID;
        os << " - " << std::get<ErrorCode>(response.m_header.info);
        os << " ( on function: " << std::get<ErrorFunctionCode>(response.m_header.FunctionCode) << " )";
  }
  return os;
}
...
std::vector<uint8_t> Response::toRaw() const {
  ...
  if (std::holds_alternative<FunctionCode>(m_header.FunctionCode)) {
    std::uint8_t functionCode = static_cast<std::uint8_t>(std::get<FunctionCode>(m_header.FunctionCode));
    std::uint8_t count = static_cast<std::uint8_t>(std::get<std::uint8_t>(m_header.info));
    ...
    std::uint8_t functionCode = static_cast<std::uint8_t>(std::get<ErrorFunctionCode>(m_header.FunctionCode));
    std::uint8_t count = static_cast<std::uint8_t>(std::get<ErrorCode>(m_header.info));
    result.emplace_back(functionCode);
    result.emplace_back(count);
  }
